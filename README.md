...
- [ ] 部署MinerU 教程
- [ ] 部署PaddleOCR  教程
- [ ] 实现 MinerU 、PaddleOCR 解析工具的封装代码，支持多格式文档解析
- [ ] 给出MinerU  与 PaddleOCR 的对比测试代码，验证图文混合文档解析精度
...
- [ ] 集成 Qwen-7B-Chat、ChatGLM3-6B 本地模型
- [ ] 现闭源 API 调用
...
- [ ] 实现 Query 与历史上下文的融合逻辑，支持将历史对话摘要（LLM 生成摘要）与当前 Query 合并
- [ ] 新增对话状态保存功能，使用 Redis 缓存对话历史（key 为用户 ID，value 为对话列表），支持过期时间设置
- [ ] 补充历史对话截断策略，当历史对话总长度超过 1024 token 时，使用 LLM 生成对话摘要替换原始历史，保留核心上下文信息
